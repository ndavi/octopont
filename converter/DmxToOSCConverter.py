...
class DmxToOSCConverter(object):
    ...
    def convertToStrobeMsg(self):
        ...
        if (self.dmxArray[self.configStrobeSend[0]] == 0 and doConvert1):
            values = [1]
            self.osc.sendDefault(strMsg, target,
                                 [self.configStrobeSend[0] + 1, self.dmxArray[self.configStrobeSend[0]]], values)
        ...
        if (doConvert2):
            ...
            self.osc.sendDefault(strMsg, "VIDEO",
                                 [self.configStrobeSend[1] + 1, self.dmxArray[self.configStrobeSend[1]]], values)
